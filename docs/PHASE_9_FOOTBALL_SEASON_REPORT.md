...
- **DB-Integrationstests** (`packages/database/src/__tests__/football-season.integration.spec.ts`, neu, 20 Tests): Cross-Tenant Season↔Department, Cross-Tenant TeamSeason↔Team/Season/AgeGroup, Datumsbereichs-CHECK, zweite-ACTIVE-Saison-Ablehnung, `@@unique([teamId, seasonId])`-Ablehnung, RLS fail-closed ohne Tenant-Kontext (Season/AgeGroup/TeamSeason), Tenant-Isolation, AgeGroup-Eindeutigkeit je Tenant. **Real gegen PostgreSQL 17 verifiziert** (siehe Abschnitt 18): 20/20 grün, zusammen mit den bestehenden Suiten (RLS/Cross-Tenant-FK) 53/53 Tests der Paket-Integrationssuite grün — keine Regression durch die `tenant-prisma.ts`-Änderung (Abschnitt 11).
- **API-Integrationstests** (`apps/api/test/football-season.integration-spec.ts`, neu, 20 Tests): 401 ohne Session, 403 ohne Mitgliedschaft, TENANT_ADMIN/DEPARTMENT_ADMIN-Fußball/COACH-Rollenmatrix für Season (create/update/read, inkl. expliziter Ablehnung DEPARTMENT_ADMIN-Fußball → Tennis-Saison), Datumsvalidierung, zweite-ACTIVE-Ablehnung (409), AgeGroups-Rollenprüfung (Lesen für jede Rolle, Anlegen nur TENANT_ADMIN), TeamSeason inkl. Fußball-only-Guardrail (400 bei Tennis-Team) und COACH-E1-vs-E2-Isolation. **Realer Fund während der VPS-Verifikation**: ein Test ("TENANT_ADMIN can create a team season for a football team") ließ eine erzeugte `TeamSeason`-Zeile ohne Cleanup zurück; ein späterer Test erzeugte für dasselbe Team/Saison-Paar eine zweite Zeile und verletzte den `@@unique([teamId, seasonId])`-Constraint — reiner Test-Isolationsfehler, keine Anwendungslogik betroffen. Behoben durch expliziten Cleanup (`adminPrisma.teamSeason.delete`). Danach 20/20, gesamte API-Integrationssuite (7 Dateien) 85/85 grün.
...
- **E2E** (`apps/web/e2e/football-season.spec.ts`, neu, 3 Tests): Login DEV-Admin (bestehendes TENANT_ADMIN-Fixture) → Verein → Fußball (Nav) → Saison 2026/2027 sichtbar → E1/E2 mit Altersklasse E-Jugend sichtbar → Saisons verwalten → neue Saison anlegen → bearbeiten; zusätzlich COACH-E1-Unterfall (liest aktive Saison und eigenes Team, keine Saisonverwaltung sichtbar/nutzbar). Kein eigenes DEPARTMENT_ADMIN-E2E-Fixture ergänzt — das bestehende `global-setup.ts` stellt nur TENANT_ADMIN und COACH bereit; die DEPARTMENT_ADMIN-Rollenlogik ist bereits vollständig durch die API-Integrationstests abgedeckt. **Zwei reale Funde während der VPS-Verifikation, beide behoben**: (1) `getByText("Aktiv")`/`getByText("Geplant")` trafen sowohl auf das sichtbare Status-Badge als auch auf `<option>`-Text im (geschlossenen) Status-`<select>` desselben Elements — auf `span.rounded-full`-Badge-Locator umgestellt. (2) der Saisonname `"2027/2028"` war nicht über mehrere Testläufe hinweg eindeutig (keine DELETE-Route existiert, um ihn danach zu entfernen) — auf einen zeitstempelbasierten, pro Lauf eindeutigen Namen umgestellt (`E2E Saison ${Date.now()}`), analog zum bestehenden Muster in `global-setup.ts`. Nach beiden Fixes: alle 3 Tests über mehrere komplette Testlaufwiederholungen hinweg durchgehend grün.

## 17. CI

Lokale Quality Gates vollständig grün: `pnpm install --frozen-lockfile`, `pnpm lint` (7 Pakete), `pnpm typecheck` (7 Pakete), `pnpm test` (alle DB-unabhängigen Unit-Tests: 5 + 52 + 61 = 118 Tests grün), `pnpm build` (inkl. `next build` mit den neuen Routen `/fussball`, `/fussball/saisons`), `prisma validate`. Die DB-abhängigen Integrations-/E2E-Suiten liefen real gegen PostgreSQL 17 auf dem VPS, siehe Abschnitt 18.

## 18. DEV-Deployment-Status

**Reale VPS-Verifikation durchgeführt und abgeschlossen.** Sitzungsgebundener, temporärer SSH-Key generiert, vom Nutzer manuell in `~/.ssh/authorized_keys` hinterlegt und bestätigt. Danach:

1. Temporärer, eindeutig gekennzeichneter PostgreSQL-17-Container (`verevia-tmp-dev-postgres-phase9-football`, Label `verevia.purpose=temporary-phase9-football-dev`) auf `verevia-dev`, ausschließlich an `127.0.0.1:5440` des VPS gebunden. Zugriff per SSH-Local-Port-Forward (`-L 5440:127.0.0.1:5440`); alle folgenden Schritte liefen mit der lokal installierten Prisma-CLI/Vitest/Playwright-Toolchain gegen diese getunnelte Verbindung.
2. Alle 9 Migrationen (inkl. der neuen `20260823120000_add_football_season_foundation`) erfolgreich aus einer **leeren** Datenbank angewendet (`prisma migrate deploy`).
3. Schema-Drift-Prüfung: `prisma migrate diff --exit-code` → **kein Unterschied**.
4. Seed zweimal ausgeführt — beim zweiten Lauf identische IDs, keine Duplikate (verifiziert: genau 1 Season, 1 AgeGroup, 2 TeamSeason-Zeilen nach zwei Läufen).
5. DB-Integrationstests: 53/53 grün (siehe Abschnitt 16).
6. API-Integrationstests: 85/85 grün nach dem in Abschnitt 16 beschriebenen Fix (7 Testdateien).
7. `apps/api`/`apps/web` produktiv gebaut und als echte Prozesse gestartet (Port 3001/3100 — 3000 war durch ein anderes, unabhängiges lokales Projekt belegt und wurde nicht angetastet), gegen die getunnelte DB.
8. Volle Playwright-E2E-Suite mehrfach ausgeführt: alle 4 neuen Fußball-Tests (Abschnitt 16) über mehrere Wiederholungen durchgehend grün; die 4 bestehenden, nicht von Phase 9 fachlich berührten Testdateien (`club-structure`, `guardian-invitation`, `role-management`, `team-membership`) zeigten die bereits in `playwright.config.ts` dokumentierte, bekannte Timing-Flakiness unter SSH-Tunnel-Latenz zur VPS-DB (unterschiedliche einzelne Tests schlugen bei Volllauf-Wiederholungen abwechselnd fehl) — **jeder einzelne dieser Tests lief isoliert ausgeführt zuverlässig grün**, und die zugrundeliegenden DB-Schreibvorgänge waren in jedem Fall korrekt (direkt in der DB verifiziert), sodass dies als reine Umgebungs-Latenz-Flakiness des temporären SSH-Tunnel-Verfahrens eingeordnet wird, nicht als Phase-9-Regression.
9. Vollständig aufgeräumt: temporärer Postgres-Container entfernt (`docker rm -f`), anhängendes anonymes Volume entfernt (verifiziert: `docker volume ls -qf dangling=true` → 0 Treffer, nur das dauerhafte `docker_verevia-dev-postgres-data`-Volume bleibt), SSH-Tunnel geschlossen (verifiziert: Port 5440 lokal nicht mehr erreichbar), lokale `api`/`web`-Prozesse beendet, temporärer SSH-Key aus `authorized_keys` entfernt und die Entfernung durch einen fehlschlagenden erneuten Verbindungsversuch mit genau diesem Key verifiziert (`Permission denied`), lokale Schlüsseldateien gelöscht. `verevia-prod`, Traefik, DNS, dauerhafte SSH-Konfiguration und die laufende DEV-Datenbank wurden zu keinem Zeitpunkt verändert.

**Kleiner, unschädlicher Rückstand** (identisches Muster zu Phase 6/7): `~/.ssh/authorized_keys.bak` auf dem VPS (von `sed -i.bak` beim Entfernen des temporären Schlüssels erzeugt) konnte nach dem Zugriffsentzug nicht mehr selbst gelöscht werden — enthält ausschließlich den bereits entfernten, öffentlichen Schlüsseltext, kein aktives Credential. Exakter Befehl für den Nutzer, falls gewünscht: `rm ~/.ssh/authorized_keys.bak`.

Kein tatsächliches Deployment auf die permanente DEV-Umgebung (`app.verevia.app`/`api.verevia.app`) in diesem Schritt — das erfolgt automatisch erst nach Merge des Phase-9-PR in `main` durch die bestehende, seit Phase 8 automatisierte Pipeline (Abschnitt 21 des Auftrags).
...
- Die bekannte, bereits vor Phase 9 dokumentierte E2E-Timing-Flakiness unter SSH-Tunnel-Latenz (Abschnitt 18, Punkt 8) betrifft weiterhin gelegentlich bestehende, nicht-Fußball-bezogene Tests bei einem vollständigen seriellen Suite-Lauf gegen eine getunnelte VPS-DB — kein Phase-9-spezifisches Risiko, aber unverändert ein offener Punkt für eine künftige Verbesserung der Testumgebung (z. B. eine näher an der Produktion liegende, nicht getunnelte DB-Verbindung für CI-artige Läufe).
...
PR (`feat(football): add season and team season foundation`) durchsehen und freigeben — **ohne Auto-Merge**, wie in jeder vorherigen Phase. Nach Merge: automatische Pipeline (CI → GHCR → DEV-Rollout) beobachten und gegen die reale DEV-Umgebung (`app.verevia.app`/`api.verevia.app`) verifizieren (Login → Verein → Fußball → aktive Saison → E1 → Altersklasse; `/health` mit neuer Git-SHA; bestehende Phase-8-Daten unverändert). Ausdrücklich weiterhin **nicht** Teil dieser Phase: Turnierplaner, Spiele, Training, Anwesenheit, Termine, Spielgemeinschaften, Tabellen/Liga, Verbandsschnittstellen, Kaderhistorie.
