...
| Event (Termin) | Ein Kalendereintrag, z. B. Training oder Besprechung. Weiterhin nicht implementiert. |
| Attendance (Anwesenheit) | Zu- oder Absage sowie tatsächliche Anwesenheit einer Person zu einem `Event`. Weiterhin nicht implementiert. |
| Task (Aufgabe) | Eine einer Mannschaft oder Person zugeordnete Aufgabe. Weiterhin nicht implementiert. |
| Tournament (Turnier) | Ein geplantes oder durchgeführtes Turnier. Weiterhin nicht implementiert. |
| TournamentTeam | Zuordnung einer teilnehmenden Mannschaft zu einem Turnier. Weiterhin nicht implementiert. |
| Match (Spiel) | **Implementiert (Phase 10) als `FootballMatch`** — kein sportneutrales Basismodell, siehe Phase-10-Hinweis oben. Referenziert `TeamSeason` (nicht `Team`+`Season` getrennt) — die Saison ist darüber eindeutig ableitbar, kein redundantes `seasonId`-Feld. Felder u. a. `venueId?`, `startsAt`, `type`, `status`, `homeAway`, `opponentName`, `homeScore?`/`awayScore?`. Ein künftiger Turnierplaner referenziert perspektivisch dasselbe Modell, kein paralleles zweites Spielmodell. |
